...
const notification = (overrides: Partial<NotificationResponse> = {}): NotificationResponse => ({
  ...
  title: 'New order',
  body: 'You have received a new order 89ABCDEF.',
...
describe('NotificationsPage (vendor portal)', () => {
  ...
  it('renders the title and body the server sent', () => {
    ...
    expect(screen.getByText('You have received a new order 89ABCDEF.')).toBeInTheDocument();
  });

  it('sends the vendor to the order list, not to a sub-order route that cannot resolve', () => {
    // The payload carries the parent order id; this app's `/orders/:id` takes
    // a sub-order id. Linking to the former renders "not found" in the real
    // portal, which is why the link points at the list and says so.
    renderPage({ items: [notification()], unread: 1 });

    expect(screen.getByRole('link', { name: 'View orders' })).toHaveAttribute('href', '/orders');
    expect(screen.queryByRole('link', { name: `/orders/${ORDER_ID}` })).not.toBeInTheDocument();
  });
