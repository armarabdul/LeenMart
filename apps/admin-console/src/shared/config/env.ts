...
/**
 * Frontend environment configuration — mirrors `vendor-portal`'s own
 * `env.ts` (same validation discipline, same "nothing here may be a
 * secret" rule).
 */
const envSchema = z.object({
  VITE_API_BASE_URL: z.string().min(1).default('/api/v1'),
  VITE_APP_NAME: z.string().min(1).default('Leen Mart Admin Console'),
