 ...
 * (Phase H): every form on this app that has one validates against the exact
 * same `@leen-mart/contracts` Zod schema the API's own `validate()`
 * middleware parses the request body with — never a hand-copied regex or
 * length that could quietly go stale next to it.
 *
