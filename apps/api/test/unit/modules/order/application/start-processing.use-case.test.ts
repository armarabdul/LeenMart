...
import { StartProcessingUseCase } from '../../../../../src/modules/order/application/use-cases/start-processing.use-case.js';
...
const buildDetail = (status: OrderStatus, version = 1): VendorSubOrderDetail => ({
...
const vendorOrderRepo = (overrides: Partial<VendorOrderRepository> = {}): VendorOrderRepository => {
  const repository: VendorOrderRepository = {
    ...
    findDetailById: vi.fn().mockResolvedValue(buildDetail(OrderStatus.CONFIRMED)),
...
const buildUseCase = (overrides: BuildOverrides = {}): StartProcessingUseCase =>
  new StartProcessingUseCase({
...
describe('StartProcessingUseCase', () => {
  it('moves a CONFIRMED sub-order to PROCESSING', async () => {
    const useCase = buildUseCase();

    const result = await useCase.execute(input);

    expect(result.subOrder.status).toBe(OrderStatus.PROCESSING);
  });

  it('never touches the parent Order — no repository call reads or writes "orders" (locked decision #1)', async () => {
    const vendorOrder = vendorOrderRepo();
    const useCase = buildUseCase({ vendorOrderRepository: vendorOrder });

    await useCase.execute(input);

    // The vendor-order repository's own surface has no method that reads or
    // writes the parent Order at all — findDetailById/updateStatusIfVersionMatches
    // are the only two calls, and neither takes or returns an Order.
  ...
  it('writes with the version the sub-order was read at', async () => {
    const vendorOrder = vendorOrderRepo({
      findDetailById: vi.fn().mockResolvedValue(buildDetail(OrderStatus.CONFIRMED, 5)),
    });
    const useCase = buildUseCase({ vendorOrderRepository: vendorOrder });
  ...
  it('rejects with SubOrderConcurrentlyModifiedError when the version was already moved (locked decision #9)', async () => {
    const useCase = buildUseCase({
  ...
  it('refuses a sub-order that is not CONFIRMED (reuses SubOrder.startProcessing(), no duplicated state machine)', async () => {
    const useCase = buildUseCase({
      vendorOrderRepository: vendorOrderRepo({
        findDetailById: vi.fn().mockResolvedValue(buildDetail(OrderStatus.PENDING_PAYMENT)),
      }),
    });
  ...
  it('writes a sub_order.processing_started outbox event', async () => {
  ...
  it('writes an audit record for the transition (this module’s first audited action)', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        action: 'sub_order.processing_started',
        entityType: 'SubOrder',
      }),
    );
  });
  ...
  it('does not write outbox or audit when the transition is refused', async () => {
    ...
    const useCase = buildUseCase({
      ...
      auditWriter: audit,
      vendorOrderRepository: vendorOrderRepo({
        findDetailById: vi.fn().mockResolvedValue(buildDetail(OrderStatus.CANCELLED)),
      }),
    });
