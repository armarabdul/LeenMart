...
import { DeliverSubOrderUseCase } from '../../../../../src/modules/order/application/use-cases/deliver-sub-order.use-case.js';
...
const vendorOrderRepo = (overrides: Partial<VendorOrderRepository> = {}): VendorOrderRepository => {
  const repository: VendorOrderRepository = {
    ...
    findDetailById: vi.fn().mockResolvedValue(buildDetail(OrderStatus.SHIPPED)),
...
const buildUseCase = (overrides: BuildOverrides = {}): DeliverSubOrderUseCase =>
  new DeliverSubOrderUseCase({
...
describe('DeliverSubOrderUseCase', () => {
  it('moves a SHIPPED sub-order to DELIVERED', async () => {
    const useCase = buildUseCase();

    const result = await useCase.execute(input);

    expect(result.subOrder.status).toBe(OrderStatus.DELIVERED);
  });

  it('never touches the parent Order (locked decision #7)', async () => {
  ...
  it('writes with the version the sub-order was read at', async () => {
    const vendorOrder = vendorOrderRepo({
      findDetailById: vi.fn().mockResolvedValue(buildDetail(OrderStatus.SHIPPED, 6)),
    });
    const useCase = buildUseCase({ vendorOrderRepository: vendorOrder });
  ...
  it('rejects with SubOrderConcurrentlyModifiedError when the version was already moved (locked decision #10)', async () => {
    const useCase = buildUseCase({
  ...
  it('refuses a sub-order that is not SHIPPED (reuses SubOrder.deliver(), no duplicated state machine)', async () => {
    const useCase = buildUseCase({
      vendorOrderRepository: vendorOrderRepo({
  ...
  it('refuses a DELIVERED sub-order (terminal state, no double-delivery)', async () => {
    const useCase = buildUseCase({
      vendorOrderRepository: vendorOrderRepo({
        findDetailById: vi.fn().mockResolvedValue(buildDetail(OrderStatus.DELIVERED)),
      }),
    });
  ...
  it('writes a sub_order.delivered outbox event', async () => {
