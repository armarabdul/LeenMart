...
const buildUseCase = (overrides: BuildOverrides = {}): ShipSubOrderUseCase =>
  new ShipSubOrderUseCase({
...
describe('ShipSubOrderUseCase', () => {
  it('moves a PROCESSING sub-order to SHIPPED', async () => {
    const useCase = buildUseCase();

    const result = await useCase.execute(input);

    expect(result.subOrder.status).toBe(OrderStatus.SHIPPED);
  });
  ...
  it('writes with the version the sub-order was read at', async () => {
    const vendorOrder = vendorOrderRepo({
      findDetailById: vi.fn().mockResolvedValue(buildDetail(OrderStatus.PROCESSING, 5)),
    });
    const useCase = buildUseCase({ vendorOrderRepository: vendorOrder });
  ...
  it('refuses a sub-order that is not PROCESSING (reuses SubOrder.ship(), no duplicated state machine)', async () => {
    const useCase = buildUseCase({
      vendorOrderRepository: vendorOrderRepo({
  ...
  it('writes a sub_order.shipped outbox event', async () => {
